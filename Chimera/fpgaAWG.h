...
public:
	...
	void ampGaussianMod(unsigned long channel, float tStart, float tEnd, float tSigma, float amodfreq, float amodampfrac, float amodmax, float amodphase);
	void ampGaussianModSquare(unsigned long channel, float tStart, float tEnd, float tSigma, float freq, float amodfreq, float amodamp, float amodphase);

	//void ampGHZRamp(unsigned long channel, float tStart, float tEnd, float ampMax, float power, float omega, float norm, float offset, int N, int omm);
	double evaluatePolynomial(const std::vector<double>& coeffs, double x);
	void freqGHZRamp(unsigned long channel, float tStart, float tEnd, float freq, float omega, bool phase_update, float phaseStart, int N, int omm);
	//std::vector<double> readCoefficients(const std::string& filename);
