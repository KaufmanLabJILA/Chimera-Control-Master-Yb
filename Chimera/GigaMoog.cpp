...
void gigaMoog::writeLoad(MessageSender& ms)
{
...

	{
		...
		for (auto const& channelBool : initialPositionsX)
		{
			...
			if (iTweezer > 48)
			{
				thrower("For safety, maximum number of x tones is limited to 48 in rearrangement mode");
			}
			if (channelBool)
			{
	...
	}
	{
		...
		for (auto const& channelBool : initialPositionsY)
		{
			...
			if (iTweezer > 2)
			{
				thrower("For safety, maximum number of y tones is limited to 2 in rearrangement mode");
			}
			if (channelBool)
			{
