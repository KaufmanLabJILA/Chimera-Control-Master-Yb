...
void fpgaAWG::ampGaussianMod(unsigned long channel, float tStart, float tEnd,float tSigma, float amodfreq, float amodampfrac, float amodmax, float amodphase) {
	int iStart = nPreviousStepSetting + ceil((tStart - startTimeStepSetting) / (stepSize * AWGMINSTEP));
	int numSteps = ceil((tEnd - tStart) / (stepSize * AWGMINSTEP));
	int numTurn = ceil((tSigma) / (stepSize * AWGMINSTEP));
	...
	for (int i = 0; i < numTurn; i++) {
		t = stepSize * AWGMINSTEP * i + tStart;
		aVal = (amodmax * (amodampfrac / 2) * cos(2*3.141592*amodfreq*(t - tEnd)+ amodphase) + amodmax*(1- amodampfrac/2))* exp(-1 * 3*pow((t - (tStart+tSigma)), 2) / (2 * pow(tSigma, 2)));
		awgCommandList[iStart + i].ampPercent = aVal;
	};
	for (int i = numTurn; i < numSteps-numTurn; i++) {
		t = stepSize * AWGMINSTEP * i + tStart;
		aVal = (amodmax * (amodampfrac / 2) * cos(2 * 3.141592*amodfreq*(t - tEnd) + amodphase) + amodmax * (1 - amodampfrac / 2));
		awgCommandList[iStart + i].ampPercent = aVal;
	};
	for (int i = numSteps - numTurn; i < numSteps; i++) {
		t = stepSize * AWGMINSTEP * i + tStart;
		aVal = (amodmax * (amodampfrac / 2) * cos(2 * 3.141592*amodfreq*(t - tEnd) + amodphase) + amodmax * (1 - amodampfrac / 2))* exp(-1 * 3*pow((t - (tEnd - tSigma)), 2) / (2 * pow(tSigma, 2)));
		awgCommandList[iStart + i].ampPercent = aVal;
	};
...
void fpgaAWG::ampGaussianModSquare(unsigned long channel, float tStart, float tEnd, float tSigma, float freq,float amodfreq, float amodamp, float amodphase) {
	std::vector<awgCommand> & awgCommandList = selectCommandList(channel);
	int ts = getTTW(1/amodfreq/2);
	if (ts <= 0) {
		thrower("step size below minimum of 6.5 ns.");
	};
	stepSize = ts;
	float Initphase = amodphase -(2.0 * 3.141592)*tStart * amodfreq;
	float tPhase = fmod(Initphase , (2.0 * 3.141592))/ (2.0 * 3.141592) / amodfreq;
	if (tPhase < 0) { tPhase += 1 / amodfreq; };
	...
	float t,thalfperiod, aVal, aValfactor, phaseflag;

	if (tPhase <= ts * AWGMINSTEP) {
		phaseflag = 0;
	}
	else if (tPhase > ts* AWGMINSTEP) {
		phaseflag = 1;
	};
	...
	awgCommandList.push_back(awgCommand());
	awgCommandList.back().address = nPreviousStepSetting ;
	awgCommandList.back().timeStampMicro = tStart ; //must have initial time step at t=0.
	awgCommandList.back().ampPercent = amodamp * phaseflag;
	awgCommandList.back().freqMHz = freq;
	awgCommandList.back().phaseDegrees = 0;
	awgCommandList.back().phase_update = true;


	for (int i = 1; i < numSteps; i++) {
		awgCommandList.push_back(awgCommand());
		awgCommandList.back().address = nPreviousStepSetting + i;
		awgCommandList.back().timeStampMicro = tStart +0.007+ (tPhase- ts * AWGMINSTEP  * phaseflag)+ (i-1) * stepSize * AWGMINSTEP; // making sure not = to tStart
		t = tStart + 0.007 + (tPhase- ts * AWGMINSTEP * phaseflag) + (i - 1) * stepSize * AWGMINSTEP;

		aValfactor = (i+ int(phaseflag)) % 2;
		if (t < tStart+tSigma) {
			aVal = amodamp * exp(-1 * 5 * pow((t+ ts * AWGMINSTEP/2 - (tStart + tSigma)), 2) / (2 * pow(tSigma, 2)));
			awgCommandList.back().ampPercent = aValfactor * aVal;
		}
		else if (t < tEnd - tSigma) {
			aVal = amodamp;
			awgCommandList.back().ampPercent = aValfactor * aVal;
		}
		else if (t >= tEnd - tSigma) {
			aVal = amodamp * exp(-1 * 5 * pow((t+ ts * AWGMINSTEP/2 - (tEnd - tSigma)), 2) / (2 * pow(tSigma, 2)));
			awgCommandList.back().ampPercent = aValfactor * aVal;
		};

		awgCommandList.back().freqMHz = freq;
		awgCommandList.back().phaseDegrees = 0;
		awgCommandList.back().phase_update = false;
	};
	//very last point
	awgCommandList.push_back(awgCommand());
	awgCommandList.back().address = nPreviousStepSetting + numSteps ;
	awgCommandList.back().timeStampMicro = tEnd; //must have initial time step at t=0.
	awgCommandList.back().ampPercent = 0;
	awgCommandList.back().freqMHz = freq;
	awgCommandList.back().phaseDegrees = 0;
	awgCommandList.back().phase_update = false;
...
std::vector<double> fpgaAWG::readCoefficients(const std::string& filename) {
	...
	// Read coefficients from the file and store them in a vector
	while (inputFile >> coeff) {
...
void fpgaAWG::freqGHZRamp(unsigned long channel, float tStart, float tEnd, float freq, float omega, bool phase_update, float phaseStart, int N, int omm) {
	// In the polynomials sent by guiliano, t goes from 0 to 1
	int iStart = nPreviousStepSetting + ceil((tStart - startTimeStepSetting) / (stepSize * AWGMINSTEP));
	int numSteps = ceil((tEnd - tStart) / (stepSize * AWGMINSTEP));
	std::vector<awgCommand> & awgCommandList = selectCommandList(channel);
	char filePath[200];
	sprintf(filePath, "C:\\Users\\klab\\Documents\\Chimera-Control-Master\\Waveforms\\ampl%i_N=%i.txt", omm, N);
	...
	for (int i = 0; i < numSteps; i++) {
		t = stepSize * AWGMINSTEP * i + tStart;
		fVal = freq + omega * evaluatePolynomial(coefficients, t/tEnd);
...
void fpgaAWG::ampGHZRamp(unsigned long channel, float tStart, float tEnd, float ampMax, float power, float omega, float norm, float offset, int N, int omm) {
	...
	if (omm == 1) {
		for (int i = 0; i < numSteps; i++) {
			t = stepSize * AWGMINSTEP * i + tStart;
			fVal = omega * evaluatePolynomial(coefficients, t/tEnd);
...
void fpgaAWG::analyzeAWGScript(fpgaAWG* fpgaawg, std::vector<variableType>& variables, UINT variation, int AWGnum)
{
	...
	while (!(currentAWGScript.peek() == EOF) || word != "__end__")
	{
		...
		if (word == "customsequence") {
		...
		else if (word == "ampgaussmod") {
			std::string channel;
			Expression tstart, tstop,tsigma, amodfreq, amodampfrac, amodmax,amodphase;
			currentAWGScript >> channel;
			currentAWGScript >> tstart;
			currentAWGScript >> tstop;
			currentAWGScript >> tsigma;
			currentAWGScript >> amodfreq;
			currentAWGScript >> amodampfrac;
			currentAWGScript >> amodmax;
			currentAWGScript >> amodphase;
			ampGaussianMod(stoul(channel, nullptr, 0), tstart.evaluate(variables, variation), tstop.evaluate(variables, variation), tsigma.evaluate(variables, variation), amodfreq.evaluate(variables, variation), amodampfrac.evaluate(variables, variation), amodmax.evaluate(variables, variation), amodphase.evaluate(variables, variation));
		}

		else if (word == "ampgaussmodsq") {
			std::string channel;
			Expression tstart, tstop, tsigma, freq, amodfreq, amodamp, amodphase;
			currentAWGScript >> channel;
			currentAWGScript >> tstart;
			currentAWGScript >> tstop;
			currentAWGScript >> tsigma;
			...
			currentAWGScript >> amodfreq;
			currentAWGScript >> amodamp;
			currentAWGScript >> amodphase;
			fpgaAWG::ampGaussianModSquare(stoul(channel, nullptr, 0), tstart.evaluate(variables, variation), tstop.evaluate(variables, variation), tsigma.evaluate(variables, variation), freq.evaluate(variables, variation), amodfreq.evaluate(variables, variation), amodamp.evaluate(variables, variation), amodphase.evaluate(variables, variation));
		}
		...
		/// GHZ RAMPS ///
			...
			std::string channel;
			Expression tstart, tstop, freq, omega, phase_update, phaseDegrees, N, omm;
			currentAWGScript >> channel;
			currentAWGScript >> tstart;
			currentAWGScript >> tstop;
			currentAWGScript >> freq;
			...
			freqGHZRamp(stoul(channel, nullptr, 0), tstart.evaluate(variables, variation), tstop.evaluate(variables, variation), freq.evaluate(variables, variation), omega.evaluate(variables, variation), phase_update.evaluate(variables, variation), phaseDegrees.evaluate(variables, variation), N.evaluate(variables, variation), omm.evaluate(variables, variation));
		}
