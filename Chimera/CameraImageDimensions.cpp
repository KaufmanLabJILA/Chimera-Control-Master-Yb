...
{
	currentImageParameters.width = (currentImageParameters.right - currentImageParameters.left) / currentImageParameters.horizontalBinning;
	currentImageParameters.height = (currentImageParameters.bottom - currentImageParameters.top) / currentImageParameters.verticalBinning;
}
...
void ImageDimsControl::setImageParametersFromInput( imageParameters param, CameraWindow* camWin )
{
	...
	if (currentImageParameters.left < 1 || currentImageParameters.right > 4096)
	{
		isReady = false;
		thrower( "ERROR: Image horizontal borders must be greater than 0 and less than the detector width\r\n" );
	}
	if (currentImageParameters.bottom < 1 || currentImageParameters.top > 2304)
	{
		isReady = false;
		thrower( "ERROR: Image verttical borders must be greater than 0 and less than the detector height\r\n" );
	}
