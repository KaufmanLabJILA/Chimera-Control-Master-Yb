...
std::array<double, 32> DacSystem::getDacStatus()
{
	return dacValues;
}
...
std::array<double, 32> DacSystem::getFinalSnapshot()
{
...
std::array<std::string, 32> DacSystem::getAllNames()
{
	return dacNames;
}
 ...
 */
void DacSystem::setDacStatusNoForceOut(std::array<double, 32> status)
{
...
void DacSystem::makeFinalDataFormat(UINT variation)
{
	...
	{
		for (int dacInc = 0; dacInc < 16; dacInc++)
		{
			finalFormatDacData[variation][0].push_back(snapshot.dacValues[dacInc]);
		}
		for (int dacInc = 16; dacInc < 32; dacInc++)
		{
