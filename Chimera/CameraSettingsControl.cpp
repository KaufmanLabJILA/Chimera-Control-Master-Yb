...
void CameraSettingsControl::initialize(cameraPositions& pos, int& id, CWnd* parent, cToolTips& tooltips)
{
	...
	cameraModeCombo.AddString("Standard Scan");
	cameraModeCombo.AddString("Slow Scan");
	// cameraModeCombo.AddString("Video Mode");
	// cameraModeCombo.AddString("Fast Kinetics Mode");
	cameraModeCombo.SelectString(0, "Standard Scan");
	pos.amPos.y += 25;
	pos.videoPos.y += 25;
	pos.seriesPos.y += 25;
	runSettings.triggerMode = "Standard Scan";
	// /// EM Gain
	...
	// INSERT ABOVE
	// cooler combo
	coolerCombo.seriesPos = { pos.seriesPos.x, pos.seriesPos.y, pos.seriesPos.x + 480, pos.seriesPos.y + 800};
	coolerCombo.videoPos = { pos.videoPos.x, pos.videoPos.y,pos.videoPos.x + 480, pos.videoPos.y + 800};
	coolerCombo.amPos = { pos.amPos.x, pos.amPos.y, pos.amPos.x + 480, pos.amPos.y + 800};
	...
	pos.videoPos.y += 25;
	// fan combo
	fanCombo.seriesPos = { pos.seriesPos.x, pos.seriesPos.y, pos.seriesPos.x + 480, pos.seriesPos.y+800};
	fanCombo.videoPos = { pos.videoPos.x, pos.videoPos.y,pos.videoPos.x + 480, pos.videoPos.y+800};
	fanCombo.amPos = { pos.amPos.x, pos.amPos.y, pos.amPos.x + 480, pos.amPos.y+800};
	fanCombo.Create(NORM_COMBO_OPTIONS, fanCombo.seriesPos, parent, IDC_FAN_COMBO);
	// set options for the combo
...
{
	CString cameraMode;
	int itemIndex = cameraModeCombo.GetCurSel();
	if (itemIndex == -1)
	{
		return;
	}
	cameraModeCombo.GetLBText(itemIndex, cameraMode);
	runSettings.cameraMode = str(cameraMode);
	qcmosFriend->setSettings(runSettings);
	runSettings.acquisitionMode = 3;
	qcmosFriend->setCameraMode();
	CRect rect;
	cameraWindow->GetWindowRect(&rect);
	cameraWindow->OnSize(0, rect.right - rect.left, rect.bottom - rect.top);
}
